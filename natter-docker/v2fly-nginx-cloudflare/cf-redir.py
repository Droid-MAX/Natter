...
def main():
    ...
    print(
        f"Setting [ {cf_redirect_host} ] DNS to [ {public_ip} ] "
        f"proxied by CloudFlare..."
    )
    ...
    print(
        f"Setting [ {cf_redirect_host} ] redirecting to "
        f"[ {cf_direct_host}:{public_port} ], https={cf_redirect_to_https}..."
    )
    cf.set_redirect_rule(
        cf_redirect_host, cf_direct_host, public_port, cf_redirect_to_https
    )
...
class CloudFlareRedir:
    ...
    def set_a_record(self, name, ipaddr, proxied=False):
        ...
        if not rec_id:
            ...
            rec_id = self._update_a_record(
                zone_id, rec_id, name, ipaddr, proxied
            )
        return rec_id

    def set_redirect_rule(
        self, redirect_host, direct_host, public_port, https
    ):
        ...
        if not rule_id:
            rule_id = self._create_redir_rule(
                zone_id,
                ruleset_id,
                redirect_host,
                direct_host,
            ...
            rule_id = self._update_redir_rule(
                zone_id,
                ruleset_id,
                rule_id,
                redirect_host,
    ...
    def _update_a_record(
        self, zone_id, rec_id, name, ipaddr, proxied=False, ttl=120
    ):
        name = name.lower()
        data = self._url_req(
            f"https://api.cloudflare.com/client/v4/zones/{zone_id}"
            f"/dns_records/{rec_id}",
            data={
                "content": ipaddr,
                "name": name,
                "proxied": proxied,
    ...
    def _find_redir_rule(self, zone_id, ruleset_id, redirect_host):
        ...
        for rule_data in data["result"]["rules"]:
            if rule_data["description"] == self._get_description(
                redirect_host
            ):
            ...
            data={
                ...
                "action_parameters": {
                    "from_value": {
                        ...
                        "target_url": {
                            "expression": (
                                f'concat("{proto}://{direct_host}:'
                                f'{public_port}", http.request.uri.path)'
                            )
                        },
