...
def main():
    ...
    print(
        f"Setting {cf_domain} SRV record to {protocol} port {public_port}..."
    )
    cf.set_srv_record(
        cf_domain, public_port, service=cf_srv_service, protocol=f"_{protocol}"
    )
...
class CloudFlareDNS:
    ...
    def set_srv_record(self, name, port, service="_natter", protocol="_tcp"):
        ...
        if not rec_id:
            rec_id = self._create_srv_record(
                zone_id, name, service, protocol, port, name
            )
        else:
            rec_id = self._update_srv_record(
                zone_id, rec_id, name, service, protocol, port, name
            )
        return rec_id
    ...
    def _find_srv_record(self, zone_id, name, service, protocol):
        ...
        for rec_data in data["result"]:
            if (
                rec_data["type"] == "SRV"
                and rec_data["name"] == f"{service}.{protocol}.{name}"
            ):
                rec_id = rec_data["id"]
                return rec_id
        return None

    def _create_srv_record(
        self,
        zone_id,
        name,
        service,
