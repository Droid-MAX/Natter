# Modified from CPython 3.4.10 "argparse" module for MicroPython
...
class HelpFormatter(object):
    ...
    def __init__(
        ...
        if width is None:
            try:
                width = int(_uos.getenv("COLUMNS", "80"))
        ...
        self._whitespace_matcher = _ure.compile("\\s+")
        self._long_break_matcher = _ure.compile("\n\n\n+")
    ...
    def _findall(self, pattern, string):
        if hasattr(_ure, "findall"):
            return _ure.findall(pattern, string)
        groups = []

        def cb(m):
            match_str = m.group(0)
            groups.append(match_str)
            return match_str

        _ure.sub(pattern, cb, string)
        return groups
    ...
    def _format_usage(self, usage, actions, groups, prefix):
        ...
        # if optionals and positionals are available, calculate usage
            ...
            if len(prefix) + len(usage) > text_width:
                ...
                opt_parts = self._findall(part_regexp, opt_usage)
                pos_parts = self._findall(part_regexp, pos_usage)
    ...
    def _format_actions_usage(self, actions, groups):
        ...
        open = r"[\[(]"
        close = r"[\])]"
        text = _ure.sub(r"(%s) " % open, r"\1", text)
        text = _ure.sub(r" (%s)" % close, r"\1", text)
        text = _ure.sub(r"%s *%s" % (open, close), r"", text)
        text = _ure.sub(r"\(([^|]*)\)", r"\1", text)
        text = text.strip()
    ...
    def _text_wrap(self, text, width):
        lines = []
        while text:
            line = text[:width]
            pos = line.rfind(" ")
            if len(text) > width and pos > 0:
    ...
    def _split_lines(self, text, width):
        ...
        return self._text_wrap(text, width)

    def _fill_text(self, text, width, indent):
        text = self._whitespace_matcher.sub(" ", text).strip()
        return "\n".join(self._text_wrap(text, width))
...
class _VersionAction(Action):
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        parser._print_message(formatter.format_help(), _usys.stdout)
        parser.exit()
...
class _SubParsersAction(Action):
    ...
    def __init__(
        ...
        self._name_parser_map = _ucollections.OrderedDict()
    ...
    def __call__(self, parser, namespace, values, option_string=None):
        ...
        if arg_strings:
            namespace.__dict__.setdefault(_UNRECOGNIZED_ARGS_ATTR, [])
...
class _ActionsContainer(object):
    def __init__(
        ...
        self._negative_number_matcher = _ure.compile(r"^-\d+$|^-\d*\.\d+$")
...
class ArgumentParser(_AttributeHolder, _ActionsContainer):
    ...
    def __init__(
        ...
        if prog is None:
            prog = _usys.argv[0]
            pos = max(prog.rfind("/"), prog.rfind("\\")) + 1
            prog = prog[pos:]
    ...
    def _get_positional_actions(self):
        return [
            action for action in self._actions if not action.option_strings
        ]
    ...
    def _match_argument(self, action, arg_strings_pattern):
        ...
        match = _ure.match(nargs_pattern, arg_strings_pattern)
    ...
    def _match_arguments_partial(self, actions, arg_strings_pattern):
        ...
        for i in range(len(actions), 0, -1):
            ...
            match = _ure.match(pattern, arg_strings_pattern)
    ...
    def exit(self, status=0, message=None):
        if message:
            self._print_message(message, _usys.stderr)
        _usys.exit(status)
